...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  revalidateCoupon: async () => {
    ...
    try {
      ...
      set({ discount: res.discount, total: subtotal - res.discount });
    } catch {
  ...
  addItem: async (payload) => {
    ...
    try {
      ...
      set({ items: data.items, subtotal: data.subtotal, total: data.subtotal - get().discount });
  ...
  updateQuantity: async (productId, variantId, quantity) => {
    ...
    const newSubtotal = calcSubtotal(optimistic);
    set({ items: optimistic, subtotal: newSubtotal, total: newSubtotal - get().discount, updatingKey: key });

    try {
      const data = await CartService.update({ productId, variantId, quantity });
      set({ items: data.items, subtotal: data.subtotal, total: data.subtotal - get().discount, updatingKey: null });
      get().revalidateCoupon();
    } catch {
      set({ items: prev, subtotal: calcSubtotal(prev), updatingKey: null });
