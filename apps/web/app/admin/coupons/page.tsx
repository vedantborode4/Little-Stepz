...
export default function AdminCouponsPage() {
  ...
  const StatusChip = ({ c }: { c: AdminCoupon }) => (
    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${isExpired(c) ? "bg-gray-100 text-gray-500" : isActive(c) ? "bg-green-50 text-green-700" : "bg-red-50 text-red-500"}`}>
              ...
              <tbody>
                {coupons.map(c => (
                  <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50/50 transition">
                    <td className="p-4"><span className="font-mono text-xs bg-gray-100 px-2.5 py-1.5 rounded-lg font-semibold tracking-wide">{c.code}</span></td>
                    <td className="p-4 text-gray-600">{c.type}</td>
                    <td className="p-4 font-semibold text-gray-900">{c.type==="PERCENTAGE" ? `${c.value}%` : `₹${c.value}`}{c.minOrderValue && <span className="text-xs text-gray-400 ml-1">min ₹{c.minOrderValue}</span>}</td>
                    <td className="p-4 text-gray-600">{c.usedCount} / {c.usageLimit ?? "∞"}</td>
                    <td className="p-4 text-gray-500 text-xs">{c.validUntil ? new Date(c.validUntil).toLocaleDateString("en-IN", { day:"numeric", month:"short", year:"numeric" }) : "No expiry"}</td>
                    <td className="p-4"><StatusChip c={c} /></td>
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => setFormModal(c)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition"><Pencil size={14} /></button>
                ...
                {!coupons.length && (<tr><td colSpan={7} className="py-16 text-center text-gray-400">No coupons yet</td></tr>)}
              </tbody>
            </table>
          </div>
            ...
            {coupons.map(c => (
              <div key={c.id} className="p-4 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-sm bg-gray-100 px-2.5 py-1 rounded-lg font-bold tracking-wide">{c.code}</span>
                  <StatusChip c={c} />
                </div>
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={() => setFormModal(c)} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition"><Pencil size={14} /></button>
                  <button onClick={() => setDeleteId(c.id)} className="p-2 rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-500 transition"><Trash2 size={14} /></button>
                </div>
              </div>
            ))}
            {!coupons.length && (<div className="py-16 text-center text-gray-400 text-sm">No coupons yet</div>)}
          </div>
