      ...
      <div className="flex items-center gap-1 bg-surface-2 p-1 rounded-xl w-full sm:w-fit overflow-x-auto scrollbar-none">
        {TABS.map(t => (
          <button key={t.value} onClick={() => handleTabChange(t.value)}
            className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition whitespace-nowrap ${tab === t.value ? "bg-surface text-text shadow-sm" : "text-muted hover:text-muted"}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="bg-surface border border-border rounded-2xl overflow-hidden">
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {affiliates.map(a => (
                  <tr key={a.id} className="border-t border-border hover:bg-surface-2/50 transition">
                    ...
                    <td className="p-4"><div><p className="font-medium text-text">{a.user.name}</p><p className="text-xs text-faint">{a.user.email}</p></div></td>
                    <td className="p-4"><span className="font-mono text-xs bg-surface-2 px-2 py-1 rounded-lg">{a.referralCode}</span></td>
                    <td className="p-4 text-muted">{(a.commissionRate * 100).toFixed(0)}% · {a.commissionType}</td>
                    <td className="p-4"><div><p className="font-semibold text-text">₹{a.totalCommission.toLocaleString()}</p><p className="text-xs text-faint">Pending: ₹{a.pendingBalance.toLocaleString()}</p></div></td>
                    <td className="p-4"><AffiliateStatusBadge status={a.status} /></td>
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/affiliates/${a.id}`} className="p-1.5 rounded-lg hover:bg-surface-2 text-faint hover:text-muted transition"><Eye size={14} /></Link>
                        {a.status === "PENDING" && (
                          <>
                            ...
                            <button onClick={() => { setModalAffiliate(a); setModalAction("reject") }} className="px-3 py-1.5 border border-red-200 dark:border-red-500/30 text-red-500 dark:text-red-400 rounded-lg text-xs font-medium hover:bg-red-50 dark:hover:bg-red-500/15">Reject</button>
                          </>
                        )}
                      </div>
                    </td>
                ...
                {!affiliates.length && (<tr><td colSpan={7} className="py-16 text-center text-faint">No affiliates found</td></tr>)}
              </tbody>
            </table>
          </div>
          ...
          <div className="sm:hidden divide-y divide-border">
            {affiliates.map(a => (
              <div key={a.id} className="p-4 space-y-2.5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-medium text-text text-sm">{a.user.name}</p>
                    <p className="text-xs text-faint">{a.user.email}</p>
                  </div>
                  <AffiliateStatusBadge status={a.status} />
                </div>
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className="font-mono bg-surface-2 px-2 py-1 rounded-lg">{a.referralCode}</span>
                  <span className="text-muted">{(a.commissionRate * 100).toFixed(0)}% · {a.commissionType}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-semibold text-text text-sm">₹{a.totalCommission.toLocaleString()}</span>
                    <span className="text-xs text-faint ml-2">Pending: ₹{a.pendingBalance.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Link href={`/admin/affiliates/${a.id}`} className="p-1.5 rounded-lg hover:bg-surface-2 text-faint"><Eye size={14} /></Link>
                    {a.status === "PENDING" && (
                      <>
                        ...
                        <button onClick={() => { setModalAffiliate(a); setModalAction("reject") }} className="px-2.5 py-1 border border-red-200 dark:border-red-500/30 text-red-500 dark:text-red-400 rounded-lg text-xs font-medium">Reject</button>
                      </>
                    )}
                  </div>
                </div>
            ...
            {!affiliates.length && (<div className="py-16 text-center text-faint text-sm">No affiliates found</div>)}
          </div>
