...
const statusBadge = (s: string) => {
  const map: Record<string, string> = { PENDING: "bg-yellow-50 text-yellow-700", APPROVED: "bg-blue-50 text-blue-700", PAID: "bg-green-50 text-green-700", CANCELLED: "bg-red-50 text-red-500" }
      ...
      {loading ? <TableSkeleton rows={10} cols={7} /> : (
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
              ...
              <tbody>
                {commissions.map(c => (
                  <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50/50 transition">
                    <td className="p-4"><p className="font-medium text-gray-900">{c.affiliate.user.name}</p><p className="text-xs text-gray-400 font-mono">{c.affiliate.referralCode}</p></td>
                ...
                {!commissions.length && (<tr><td colSpan={7} className="py-16 text-center text-gray-400">No commissions found</td></tr>)}
              </tbody>
            </table>
          </div>
          ...
          <div className="sm:hidden divide-y divide-gray-100">
            {commissions.map(c => (
              <div key={c.id} className="p-4 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-medium text-gray-900 text-sm">{c.affiliate.user.name}</p>
                    <p className="text-xs text-gray-400 font-mono">{c.affiliate.referralCode}</p>
                  </div>
                  {statusBadge(c.status)}
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <span className="text-gray-500">Order: ₹{c.order.total.toLocaleString()}</span>
                  <span className="font-bold text-gray-900">Commission: ₹{c.amount.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-400">{new Date(c.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</span>
            ...
            {!commissions.length && (<div className="py-16 text-center text-gray-400 text-sm">No commissions found</div>)}
          </div>

          {pagination.pages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              <p className="text-xs sm:text-sm text-gray-500">Page {page} of {pagination.pages} · {pagination.total} total</p>
              <div className="flex gap-2">
        ...
        <AdminModal title="Mark Commission as Paid" onClose={() => setPayModal(null)} width="max-w-sm">
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input value={transactionRef} onChange={e => setTransactionRef(e.target.value)} placeholder="UTR/Reference number" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"/>
            </div>
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">Note (optional)</label>
              <input value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Paid via UPI" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"/>
            </div>
