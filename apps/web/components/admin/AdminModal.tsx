...
export default function AdminModal({ title, onClose, children, width = "max-w-lg" }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div
        className={`bg-surface w-full ${width} shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[90vh] rounded-t-2xl sm:rounded-2xl`}
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 sm:px-6 py-3 sm:py-4 border-b border-border shrink-0">
          <h2 className="text-sm sm:text-base font-semibold text-text">{title}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-surface-2 transition text-muted">
            <X size={16} />
          </button>
        </div>
