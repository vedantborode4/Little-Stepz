...
export default function OrderActions({ order, refresh }: Props) {
  ...
  return (
    <>
      <select
        className="border border-border rounded-lg px-2 py-1.5 text-sm text-muted focus:outline-none focus:ring-2 focus:ring-primary/20"
        defaultValue=""
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-surface rounded-2xl shadow-2xl p-6 w-96">
            <h2 className="text-base font-semibold mb-3 text-text">Confirm Status Change</h2>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setIsOpen(false); setSelectedStatus(null) }}
                className="px-4 py-2 rounded-xl border border-border text-sm text-muted hover:bg-surface-2"
                disabled={loading}>
                Cancel
              </button>
