...
export default function AdminProductsTable() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      if (search.trim()) {
        ...
        setData(res.products ?? []); setPages(1); setTotal(res.products?.length ?? 0); return
      }
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      setData(p => p.filter(x => x.id !== deleteId)); setDeleteId(null)
      ...
      <AdminPageHeader
        ...
        action={
          <button onClick={() => router.push("/admin/products/new")}
            ...
            <Plus size={15} /> <span className="hidden xs:inline">Add Product</span><span className="xs:hidden">Add</span>
          </button>
        }
      />
