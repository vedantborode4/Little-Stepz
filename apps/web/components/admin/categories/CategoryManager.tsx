  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="hidden sm:block bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
                ...
                <tr className="border-t border-gray-100 hover:bg-gray-50/50 transition">
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => { setSelected(c); setModalMode("edit") }} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition"><Pencil size={14}/></button>
                      <button onClick={() => setDeleteId(c.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition"><Trash2 size={14}/></button>
                    </div>
                  </td>
                </tr>
                {children(c.id).map(child => (
                  <tr key={child.id} className="border-t border-gray-100 hover:bg-gray-50/50 transition bg-gray-50/30">
                    <td className="p-4 pl-10"><div className="flex items-center gap-2"><span className="text-gray-300">└</span><span className="text-gray-700">{child.name}</span></div></td>
                    <td className="p-4 text-gray-500 font-mono text-xs">{child.slug}</td>
                    <td className="p-4 text-gray-500 text-xs">{c.name}</td>
                    <td className="p-4 text-gray-400">—</td>
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => { setSelected(child); setModalMode("edit") }} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition"><Pencil size={14}/></button>
                        <button onClick={() => setDeleteId(child.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition"><Trash2 size={14}/></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {!categories.length && (<tr><td colSpan={5} className="py-16 text-center text-gray-400">No categories yet</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      <div className="sm:hidden space-y-2">
          ...
          <div key={c.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                {children(c.id).length > 0 && <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{children(c.id).length} sub</span>}
                <button onClick={() => { setSelected(c); setModalMode("edit") }} className="p-1.5 rounded-xl hover:bg-gray-100 text-gray-400"><Pencil size={13}/></button>
                <button onClick={() => setDeleteId(c.id)} className="p-1.5 rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={13}/></button>
              </div>
            </div>
            {children(c.id).length > 0 && (
              <div className="border-t border-gray-100 divide-y divide-gray-100">
                {children(c.id).map(child => (
                  <div key={child.id} className="flex items-center justify-between px-4 py-3 bg-gray-50/50">
                    <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => { setSelected(child); setModalMode("edit") }} className="p-1.5 rounded-xl hover:bg-gray-100 text-gray-400"><Pencil size={13}/></button>
                      <button onClick={() => setDeleteId(child.id)} className="p-1.5 rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={13}/></button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {!categories.length && (<div className="bg-white border border-gray-200 rounded-2xl py-16 text-center text-gray-400 text-sm">No categories yet</div>)}
      </div>
