...
export default function PriceTag({ prices, size = "sm", className = "" }: Props) {
  ...
  const main = size === "lg" ? "font-manrope text-3xl font-bold text-text" : "font-manrope text-primary font-semibold text-sm sm:text-base"
  const struck = size === "lg" ? "font-manrope text-base text-faint line-through" : "font-manrope text-xs text-faint line-through"
