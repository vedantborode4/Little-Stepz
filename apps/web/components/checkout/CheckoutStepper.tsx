    ...
    <div className="space-y-4">
          ...
          return (
            <div key={num} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1.5 min-w-[60px]">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200
                  ${done   ? "bg-green-500 text-white shadow-sm" :
                    active ? "bg-primary text-white ring-4 ring-primary/20 shadow-sm" :
              ...
              {idx < stepMeta.length - 1 && (
                <div className={`flex-1 h-0.5 mx-2 mb-5 rounded transition-all duration-300
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          ...
          {step > 1 && (
            <button onClick={() => goToStep(1)} className="text-primary text-sm font-medium hover:underline">
              Change
            </button>
          )}
        </div>
      ...
      {step >= 2 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            {step > 2 && (
              <button onClick={() => goToStep(2)} className="text-primary text-sm font-medium hover:underline">
                Change
              </button>
            )}
          </div>
      ...
      {step >= 3 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center mb-4">
            <h2 className="font-semibold text-gray-900 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full text-xs flex items-center justify-center font-bold bg-primary text-white">
                3
              </span>
              Payment Method
            </h2>
...
function AddressConfirmed() {
  ...
  if (!addr) return <p className="text-sm text-gray-500 flex items-center gap-1.5"><CheckCircle size={14} className="text-green-500" /> Address selected</p>
