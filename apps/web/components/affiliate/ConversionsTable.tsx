...
export default function ConversionsTable() {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div>
        <h1 className="text-lg sm:text-xl font-semibold text-text">Conversions</h1>
        <p className="text-xs sm:text-sm text-muted mt-0.5">Orders placed through your referral link</p>
      </div>

      <div className="bg-surface border border-border rounded-2xl overflow-hidden">
        {!conversions?.length ? (
          <div className="py-16 text-center">
            <div className="w-12 h-12 bg-violet-50 dark:bg-violet-500/15 rounded-2xl flex items-center justify-center mx-auto mb-3">
              <ArrowLeftRight size={22} className="text-violet-400" />
            </div>
            <p className="text-sm font-medium text-muted">No conversions yet</p>
            <p className="text-xs text-faint mt-1">Conversions appear when referred users place orders</p>
          </div>
        ) : (
          <>
            {/* Desktop table */}
              ...
              {conversions.map((c: any) => (
                <div key={c.id} className="p-4 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted">Total: ₹{c.order?.total?.toLocaleString("en-IN") ?? "—"}</span>
                    <span className="font-semibold text-green-600 dark:text-green-400">+₹{c.commission?.toLocaleString("en-IN") ?? "—"}</span>
                  </div>
